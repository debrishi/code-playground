...
t python timeout '{"language":"python","code":"while True: pass"}' 'TIME_LIMIT_EXCEEDED'
t python oom     '{"language":"python","code":"x=\"a\"*(400*1024*1024)"}' 'MEMORY_LIMIT_EXCEEDED'
...
t cpp  compile  '{"language":"cpp","code":"int main(){ bogus(); }"}' 'COMPILATION_ERROR'
t cpp  timeout  '{"language":"cpp","code":"int main(){while(1);}"}' 'TIME_LIMIT_EXCEEDED'
t cpp  oom      '{"language":"cpp","code":"#include <vector>\nint main(){ std::vector<long> v(200000000,1); }"}' 'MEMORY_LIMIT_EXCEEDED'
...
t java compile  '{"language":"java","code":"public class Main{ oops }"}' 'COMPILATION_ERROR'
t java oom      '{"language":"java","code":"public class Main{public static void main(String[] a){long[] x=new long[200_000_000];}}"}' 'MEMORY_LIMIT_EXCEEDED'

echo; echo "=== TYPESCRIPT ==="
t ts   success  '{"language":"typescript","code":"const m: string = \"hi\"; console.log(m);"}' '"statusCode": 200'
t ts   timeout  '{"language":"typescript","code":"while(true){}"}' 'TIME_LIMIT_EXCEEDED'
t ts   oom      '{"language":"typescript","code":"const a:number[]=[]; while(true)a.push(Math.random());"}' 'MEMORY_LIMIT_EXCEEDED'
